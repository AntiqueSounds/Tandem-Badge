...
bool getName(){
 ...
 // wifiPassword=String((char *)input);
  ...
  Serial.print("Enter weather location longitude (e.g. 44.123)");
  len = getResponse();
  if (len < 1) {
    longitude=43.2633161;
  }
  else{
  longitude=input.toFloat();
  Serial.println(input);
  };  
  Serial.print("Enter weather location latitude (e.g. -2.91)");
  len = getResponse();
  if (len < 1) {
    latitude=-2.9747642;
  }
  else{
  latitude=input.toFloat();
  Serial.println(input);
  };
...
bool valid(){
  ...
  Serial.printf("Timezone offest=%d\n",TZ);
  Serial.printf("Weather location  Longitude=%e Latitude=%e\n",longitude, latitude);
...
//    prefs.getUInt("Empno",empNo);
  ...
  if (prefs.begin("credentials")){
    ...
    prefs.getInt("TZ",TZ);
    prefs.getFloat("longitude",longitude);
    prefs.getFloat("latitude",latitude);
...
void readMem(){
 if (prefs.begin("name",true)){
  ...
  if (prefs.begin("credentials",true)){
    ...
    TZ = prefs.getInt("TZ");
    longitude = prefs.getFloat("longitude");
    latitude = prefs.getFloat("latitude");
    prefs.end(); // close for credentials
